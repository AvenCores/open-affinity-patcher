...
Утилита патчит `libaffinity.dll` по смещению `0x00440ED1` и заменяет:
...
- Если версия ровно `3.2.2.4557`, предупреждение не показывается
- Если версия ниже `3.2.2.4557`, выводится предупреждение о возможной некорректной работе патчера
- Если версия отличается от `3.2.2.4557`, но выше нее, предупреждение тоже будет показано
...
- Читает байты по адресу `0x00440ED1`
...
- Используйте на свой риск при патчинге версий, отличающихся от `3.2.2.4557`
